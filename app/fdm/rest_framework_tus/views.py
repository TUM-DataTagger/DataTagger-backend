...
from fdm.rest_framework_tus.parsers import TusUploadStreamParser
...
class TusCreateMixin(mixins.CreateModelMixin):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if upload_length < 0:  # Only check for negative values
        ...
        elif upload_length >= 0:  # For zero or positive values
            setattr(request, constants.UPLOAD_DEFER_LENGTH_FIELD_NAME, 0)
        ...
        # Lock the dataset so that nobody can upload new files to it until this one is complete
        if self.dataset:
            if self.dataset.locked:
                return Response(
                    data={
                        "lock": _(
                            "An upload to this dataset is in progress. You must not start a new upload until the last one is complete.",
                        ),
                    },
        ...
        # Set the pk from the url path, which should be a dataset pk
        if self.dataset:
            upload.dataset = self.dataset
            upload.save()
        ...
        # Force create temporary file
        temp_path = upload.get_or_create_temporary_file()
        upload.save()

        # Handle 0 byte files immediately
        if upload_length == 0:
            # Ensure we have a temporary file
            assert upload._temporary_file_exists()
            # Since it's 0 bytes, we can consider it fully uploaded
            upload.upload_offset = 0

            try:
                # Refresh the upload object to ensure we have the latest state
                upload.refresh_from_db()

                if upload.state == states.INITIAL:
                    # Set the uploaded_file using the temporary file before state transitions
                    with open(temp_path, "rb") as temp_file:
                        upload.uploaded_file.save(
                            upload.filename or "empty_file",
                            File(temp_file),
                            save=False,
                        )
                    upload.save()

                    # Do the transitions only if we haven't started yet
                    ...
                    # Store that we've completed our processing
                    upload._processing_complete = True

                # Send signal ONLY if we haven't processed this upload before
                if not getattr(upload, "_processing_complete", False):
...
class TusPatchMixin(mixins.UpdateModelMixin):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        # Constantly lock the dataset over and over again until the upload is complete
        if upload.dataset:
            if upload.dataset.locked and not upload.dataset.is_locked_by_myself():
                return Response(
                    data={
